  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-600 via-green-600 to-teal-600 pb-12">
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-8">
        <h1 className="text-3xl sm:text-4xl font-bold text-white mb-6 drop-shadow">Rounds in Progress</h1>
        {rounds.length === 0 && (
        ...
        <ul className="space-y-5">
          {rounds.map(round => (
            <li key={round.id} className="bg-white/95 backdrop-blur rounded-xl shadow-lg p-5 flex flex-col gap-2 border border-white/20">
              ...
              <Link href={`/round-detail?id=${round.id}`} className="inline-block w-fit bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-bold py-1.5 px-4 rounded-lg shadow transition-all text-xs sm:text-sm">Watch Round</Link>
            </li>
          ))}
        </ul>
      </div>
